...
Where:
* $S_0$: Baseline daily academic friction ($S_0  pprox 0.5$).
* $w_i, w_j$: Course credit weights (e.g., $w = 4.0$ for core lectures, $w = 2.0$ for electives).
* $\mathcal{K}_{\sigma}(	au) = \exp\left(-rac{	au^2}{2\sigma^2}
ight)$: Symmetric Gaussian kernel representing pre-exam preparation and post-exam review pressure around date $t_i$.
* $\mathcal{A}_{\lambda}(	au) = \mathbb{I}(	au \le 0) \cdot \exp\left(rac{	au}{\lambda}
ight)$: Asymmetric exponential kernel modeling deadline pressure accumulation leading up to $t_j$.

### 2. Student Fatigue Ordinary Differential Equation (ODE)
Fatigue $F(t)$ represents the cumulative psychological and cognitive strain on the student body. The rate of change $rac{dF(t)}{dt}$ is modeled via a non-linear ODE incorporating stress accumulation and calendar-governed recovery:

$$rac{dF(t)}{dt} =  lpha \cdot S(t) -  eta \cdot F(t) \cdot \Big(1 - H(t)\Big)$$

Where:
* $ lpha > 0$: Stress vulnerability coefficient ($	ext{day}^{-1}$).
* $ eta > 0$: Baseline dissipation/recovery rate ($	ext{day}^{-1}$).
* $H(t) \in [0, 1]$: Calendar recovery function defining institutional rest capacity:
  $$H(t) =  egin{cases} 
  1.0 & 	ext{if } t \in 	ext{Full Break / Holiday} \ 
  0.5 & 	ext{if } t \in 	ext{Weekend} \ 
  0.0 & 	ext{if } t \in 	ext{Instructional Day} 
  \end{cases}$$
...
$$F_{n+1} = F_n + h \sum_{i=1}^s b_i k_i, \quad k_i = f\left(t_n + c_i h, \, F_n + h \sum_{j=1}^{i-1} a_{ij} k_j
ight)$$
